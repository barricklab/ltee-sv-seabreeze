...
parser.add_argument('--plotsr', '-p', help='Path to the dir that contains the plotsr-bin file')
...
def makeplots(plotsr, syri, genomes, count, batch,length): #syri can be single file (for batch mode) or a list of file names (for series mode)
    ...
    if batch: # generate pairwise syteny plot
        ...
        path_to_plotsr=os.path.join(plotsr,'plotsr-bin')
        plotsr_command=[path_to_plotsr,'-s','500','--genomes',genomes,'--sr',syri,'-H','5','-W','10','-o',f"{prefix}_plotsr.pdf"]
        print(plotsr_command)
    ...
    if not(batch): #accept an ordered list of syri files to generate one large synteny plots in series
        ...
        prefix="file"
        path_to_plotsr=os.path.join(plotsr,'plotsr-bin')
        plotsr_command=[path_to_plotsr,'-s','500','--genomes',genomes]
        print(plotsr_command)
...
def main(batch, fasta, plotsr):
    ...
    if batch: # generate pair wise genomes files if true
        ...
        for count, name in enumerate(fasta): # pick up two sequences at  time
            ...
            makeplots(plotsr,syri,genomes,count,batch,'')
    ...
    if not(batch):
        ...
        prefix="file"+str(count)
        genomes=prefix+".genomes.tsv"
        makeplots(plotsr,syri,genomes,count,batch,str(5*len(fasta)))
